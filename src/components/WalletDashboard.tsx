...
export function WalletDashboard({ 
  ...
  return (
    <div className="min-h-screen">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className={`${isPopupMode ? 'text-lg' : 'text-xl'} font-semibold text-foreground`}>
                    {isPopupMode ? 'Octra' : 'Octra Wallet'}
                  </h1>
                      ...
                      <div className="mt-6 space-y-4">
                        ...
                        <div className="p-3 bg-muted rounded-lg">
                          ...
                          <div className="text-sm space-y-1">
                            <div>Nonce: {nonce}</div>
                            <div>{wallets.length} Wallet{wallets.length !== 1 ? 's' : ''}</div>
                          </div>
                        </div>
      ...
      {/* Main Content */}
      <main className={`octra-container ${isPopupMode ? 'py-2 px-2 pb-4' : 'py-4 px-2 sm:py-8 sm:px-4'}`}>
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4 sm:space-y-6">
          {isPopupMode ? (
            // Popup mode - 3 tabs only
            <TabsList className="grid w-full grid-cols-3 tabs-list h-auto p-0.5 sm:p-1">
